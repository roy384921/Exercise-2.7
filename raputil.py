...
def MMSE_CE(Y, pilotValue, pilotCarriers, K, P, h, SNR):
    # Calculate Least Squares (LS) estimation
    H_LS = np.zeros(K, dtype=complex)
    H_LS[pilotCarriers] = Y[pilotCarriers] / pilotValue
    
    # Interpolate if the number of pilots is less than the number of subcarriers
    if P < K:
        H_LS = interpolate(H_LS[pilotCarriers], pilotCarriers, K, 1)
        
    H_LS = H_LS.reshape(K, 1)
    
    # Convert SNR from dB to linear scale
    snr_lin = 10 ** (SNR / 10.0)
    
    # Construct time-domain covariance matrix assuming independent taps
    h_pad = np.zeros(K, dtype=complex)
    h_pad[:len(h)] = h
    R_hh = np.diag(np.abs(h_pad)**2)
    
    # Transform time-domain covariance to frequency-domain covariance matrix R_HH
    F_mat = Normalized_FFT_Matrix(K)
    R_HH = K * F_mat @ R_hh @ np.conj(F_mat).T
    
    # Calculate the MMSE weight matrix
    W_MMSE = R_HH @ np.linalg.inv(R_HH + np.eye(K) / snr_lin)
    
    # Compute the final MMSE channel estimation
    H_MMSE = (W_MMSE @ H_LS).reshape(-1,)
